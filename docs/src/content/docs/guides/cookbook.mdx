...
<Aside type="tip">
`asyncio.gather` is safe here because each delegate gets its own child ExecContext. There is no shared mutable state between parallel branches -- each has independent tracing, timeout, and memory scope.
</Aside>
...
<Aside type="note">
All Nerva streaming is based on async generators. Both the orchestrator's `stream()` method and agent handlers use `async for` / `yield`. There is no callback-based streaming API.
</Aside>
