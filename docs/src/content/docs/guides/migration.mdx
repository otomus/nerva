...
<Aside type="tip">
You don't need to migrate everything at once. Nerva primitives work standalone -- start by replacing one component (e.g., swap your router or memory layer) and expand from there.
</Aside>
...
<Aside type="note">
Nerva does not need a compile step. Unlike LangGraph's `graph.compile()`, Nerva primitives are live objects you instantiate and compose directly. There is no intermediate representation or build phase.
</Aside>
